  ...
  try {
    ...
    const { count, rows: allData } = await LmsGame.findAndCountAll({
      ...
      attributes: [
        ...
        [Sequelize.literal('(SELECT COUNT(*) FROM lmsgameviewhistory WHERE gvgameId = lmsgame.gameId)'), 'gameview'], `gameExtensionId`,
...
const OptionExitist = await lmsQuestionOptions.findOne({
  where: { qpQuestionId: result.blockId , qpSecondaryId: object.secondaryId, qpQuestNo:result.blockQuestNo,qpGameId:id },
});
 ...
 if(OptionExitist){
  setArray.push('OptionExitist'+key);
    options = await lmsQuestionOptions.update({
     ...
     {
       where: {
        qpQuestionId: result.blockId , qpSecondaryId: object.secondaryId, qpQuestNo:result.blockQuestNo,qpGameId:id 
...
WHERE
  ...
  const reviewerGameId= await ReviewersGame.findOne({where: {gameUuid : {[Op.eq]: reqUuid} }});
