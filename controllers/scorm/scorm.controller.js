...
const ScormConfiguration = require("../../models/scormConfiguration")

const createScormStructure = (tempDir, formData) => {
  // Create a folder for SCORM content
  const scormContentDir = path.join(tempDir, 'scorm_content');
  fsextra.emptyDirSync(scormContentDir);

  // Create imsmanifest.xml file
  const imscp_rootv1p1p2 = `
  <?xml version="1.0"?>
  ...
  `;
  fs.writeFileSync(path.join(scormContentDir, 'imscp_rootv1p1p2.xsd'), imscp_rootv1p1p2);

  const adlcp_rootv1p2 = `<?xml version="1.0"?>
  ...
  `;
  fs.writeFileSync(path.join(scormContentDir, 'adlcp_rootv1p2.xsd'), adlcp_rootv1p2)

  const imsmd_rootv1p2p1 = `<?xml version="1.0" encoding="UTF-8"?>
  <!-- edited by Thomas Wason  -->
  ...
  `;
  fs.writeFileSync(path.join(scormContentDir, 'imsmd_rootv1p2p1.xsd'), imsmd_rootv1p2p1);

  // Create imsmanifest.xml file
  const imsManifestContent = `
  <?xml version="1.0" encoding="UTF-8"?>
  <manifest identifier="MANIFEST-1" version="1" xmlns="http://www.imsproject.org/xsd/imscp_rootv1p1p2" xmlns:adlcp="http://www.adlnet.org/xsd/adlcp_rootv1p2" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.imsproject.org/xsd/imscp_rootv1p1p2 imscp_rootv1p1p2.xsd http://www.imsglobal.org/xsd/imsmd_rootv1p2p1 imdmd_rootv1p2p1.xsd">
    <metadata>
      <schema>ADL SCORM</schema>
      <schemaversion>${formData.scormEdition}</schemaversion>
    </metadata>
    <organizations>
  ...
  `;
  fs.writeFileSync(path.join(scormContentDir, 'imsmanifest.xml'), imsManifestContent);
  ...
  const indexHtmlContent = `
    ...
    <body>
        ...
        <p>This is a simple example of a Shareable Content Object (SCO) for SCORM.</p>

    </body>
    </html>
  `;
  fs.writeFileSync(path.join(scormContentDir, 'index.html'), indexHtmlContent);

  // Create other SCORM-specific files as needed

  // You should refer to the SCORM standard for the complete structure and required files.

  // Example: Create a sample SCO (Shareable Content Object)
  // const scoDir = path.join(scormContentDir, 'sco1');
  // fs.mkdirSync(scoDir);
  // const scoFileContent = `
  //   <!-- Your SCO content goes here -->
  // `;
  // fs.writeFileSync(path.join(scoDir, 'sco1.html'), scoFileContent);

  // Return the path to the imsmanifest.xml
  return path.join(scormContentDir, 'imsmanifest.xml');
};
...
const generateScorm = async (req,res) =>{
    try {
        ...
        // Create a temporary directory to store the SCORM package
        const tempDir = path.join(__dirname, 'temp');
        if (!fs.existsSync(tempDir)) {
          fs.mkdirSync(tempDir);
        }
    
        // Create SCORM-specific files and folders in the temporary directory
        const imsManifestPath = createScormStructure(tempDir, req.body);

        // Convert authToken to string if it's not already
        // const authTokenString = typeof authToken === 'string' ? authToken : JSON.stringify(authToken);

        // You need to implement this based on the SCORM standard and your content
        // Append the authentication token to the SCORM content
        // fs.writeFileSync(path.join(tempDir, 'authToken.txt'), authTokenString);
    
        // For example, create imsmanifest.xml, scorm_content folder, etc.
    
        // Use archiver to zip the contents
        const output = fs.createWriteStream(path.join(__dirname, 'scorm-package.zip'));
        ...
        // Append the SCORM content to the zip file
        archive.directory(tempDir, '');

        // archive.glob('**/*', {
        //   cwd: tempDir,
        //   ignore: [], // Add any patterns to ignore if necessary
        // });
        ...
        output.on('close', function () {
          res.download(path.join(__dirname, 'scorm-package.zip'), 'scorm-package.zip', (err) => {
            if (err) {
              console.error('Error sending file:', err);
            }
            // Cleanup: remove the temporary directory and zip file
            fs.rmdirSync(tempDir, { recursive: true });
            fs.unlinkSync(path.join(__dirname, 'scorm-package.zip'));
          });
        });
      } catch (error) {
...
const createOrUpdateScormConfig = async (req, res) => {
   ...
   try {
      ...
      const existingFormData = await ScormConfiguration.findOne({
        where: { gameId: gameId },
      });
  
      if (existingFormData) {
      //   // If the form data exists, update it
      const formData = {
         ...req.body,
         gameId: gameId, 
         UserId: userId, 
        ...
        await existingFormData.update(formData);
        res.status(200).send('Form data updated successfully.');
      } else {
      //   // If the form data doesn't exist, create it
      const formData = {
         ...req.body,
         gameId: gameId, 
         UserId: userId, 
        ...
        await ScormConfiguration.create(formData);
        res.status(201).send('Form data created successfully.');
      }
    } catch (error) {
      console.error('Error creating/updating form data:', error);
      res.status(500).send('Error creating/updating form data: ' + error.message);
    }
 }

const getScormConfig = async (req, res) => {
   try {
       const id = req.params.id; // Assuming id is passed in the request parameters

       // Find the SCORM configuration based on the id
       const scormConfig = await ScormConfiguration.findOne({
           where: { gameId: id }
       });

       if (scormConfig) {
           // If SCORM configuration is found, send it in the response
           res.status(200).json(scormConfig);
       } else {
           // If SCORM configuration with the provided id doesn't exist, send a 404 response
           res.status(404).send('SCORM configuration not found.');
       }
   } catch (error) {
       console.error('Error fetching SCORM configuration:', error);
       res.status(500).send('Error fetching SCORM configuration: ' + error.message);
   }
}

module.exports = {generateScorm, createOrUpdateScormConfig, getScormConfig};